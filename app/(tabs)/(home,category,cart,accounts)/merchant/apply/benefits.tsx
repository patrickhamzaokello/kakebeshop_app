import React, { useState } from "react";
import {
  StyleSheet,
  View,
  Text,
  ScrollView,
  TouchableOpacity,
  Dimensions,
} from "react-native";
import { StatusBar } from "expo-status-bar";
import { SafeAreaView } from "react-native-safe-area-context";
import { LinearGradient } from "expo-linear-gradient";
import { Ionicons, MaterialCommunityIcons } from "@expo/vector-icons";
import { router } from "expo-router";
import {
  colors,
  spacingX,
  spacingY,
  borderRadius,
  fontSize,
  fontWeight,
  shadow,
} from "@/constants/theme";

const { width } = Dimensions.get("window");

interface FeatureCardProps {
  icon: React.ReactNode;
  title: string;
  description: string;
  badge?: string;
}

const FeatureCard: React.FC<FeatureCardProps> = ({
  icon,
  title,
  description,
  badge,
}) => (
  <View style={styles.featureCard}>
    {badge && (
      <View style={styles.featureBadge}>
        <Text style={styles.featureBadgeText}>{badge}</Text>
      </View>
    )}
    <View style={styles.featureIconContainer}>{icon}</View>
    <Text style={styles.featureTitle}>{title}</Text>
    <Text style={styles.featureDescription}>{description}</Text>
  </View>
);

interface ComparisonRowProps {
  feature: string;
  kakebe: boolean | string;
  others: boolean | string;
}

const ComparisonRow: React.FC<ComparisonRowProps> = ({
  feature,
  kakebe,
  others,
}) => (
  <View style={styles.comparisonRow}>
    <Text style={styles.comparisonFeature}>{feature}</Text>
    <View style={styles.comparisonValues}>
      <View style={styles.comparisonCell}>
        {typeof kakebe === "boolean" ? (
          <Ionicons
            name={kakebe ? "checkmark-circle" : "close-circle"}
            size={24}
            color={kakebe ? colors.success : colors.error}
          />
        ) : (
          <Text style={styles.comparisonText}>{kakebe}</Text>
        )}
      </View>
      <View style={styles.comparisonCell}>
        {typeof others === "boolean" ? (
          <Ionicons
            name={others ? "checkmark-circle" : "close-circle"}
            size={24}
            color={others ? colors.success : colors.error}
          />
        ) : (
          <Text style={[styles.comparisonText, styles.comparisonTextMuted]}>
            {others}
          </Text>
        )}
      </View>
    </View>
  </View>
);

interface TestimonialProps {
  name: string;
  role: string;
  avatar: string;
  quote: string;
  stats: string;
}

const TestimonialCard: React.FC<TestimonialProps> = ({
  name,
  role,
  avatar,
  quote,
  stats,
}) => (
  <View style={styles.testimonialCard}>
    <View style={styles.quoteIconContainer}>
      <Ionicons name="chatbubble-ellipses" size={20} color={colors.primary} />
    </View>
    <Text style={styles.testimonialQuote}>"{quote}"</Text>
    <View style={styles.testimonialFooter}>
      <View style={styles.testimonialAvatar}>
        <Text style={styles.testimonialAvatarText}>{avatar}</Text>
      </View>
      <View style={styles.testimonialInfo}>
        <Text style={styles.testimonialName}>{name}</Text>
        <Text style={styles.testimonialRole}>{role}</Text>
      </View>
    </View>
    <View style={styles.testimonialStats}>
      <Ionicons name="trending-up" size={16} color={colors.success} />
      <Text style={styles.testimonialStatsText}>{stats}</Text>
    </View>
  </View>
);

interface FAQItemProps {
  question: string;
  answer: string;
  isExpanded: boolean;
  onToggle: () => void;
}

const FAQItem: React.FC<FAQItemProps> = ({
  question,
  answer,
  isExpanded,
  onToggle,
}) => (
  <TouchableOpacity
    style={styles.faqItem}
    onPress={onToggle}
    activeOpacity={0.7}
  >
    <View style={styles.faqHeader}>
      <Text style={styles.faqQuestion}>{question}</Text>
      <Ionicons
        name={isExpanded ? "chevron-up" : "chevron-down"}
        size={20}
        color={colors.textSecondary}
      />
    </View>
    {isExpanded && <Text style={styles.faqAnswer}>{answer}</Text>}
  </TouchableOpacity>
);

export default function SellerBenefitsScreen() {
  const [expandedFAQ, setExpandedFAQ] = useState<number | null>(null);

  const handleGetStarted = () => {
    router.push("/merchant/apply/signup");
  };

  const handleContactSupport = () => {
    router.push("/merchant/merchant-support");
  };

  const toggleFAQ = (index: number) => {
    setExpandedFAQ(expandedFAQ === index ? null : index);
  };

  const faqs = [
    {
      question: "How much does it cost to sell on Kakebe?",
      answer:
        "It's completely free to list your products! We only charge a small 5% commission when you make a sale. There are no monthly fees, subscription costs, or hidden charges.",
    },
    {
      question: "How do I get paid?",
      answer:
        "Once your order is confirmed and delivered, funds are transferred directly to your linked mobile money account or bank account within 24-48 hours. You can track all your earnings in the seller dashboard.",
    },
    {
      question: "What can I sell on Kakebe?",
      answer:
        "You can sell almost anything! From fashion and electronics to handmade crafts and home goods. We only restrict illegal items, counterfeit products, and items that violate our community guidelines.",
    },
    {
      question: "How do I handle shipping?",
      answer:
        "You can either arrange your own shipping or use our integrated delivery partners at discounted rates. We provide shipping labels and tracking for all orders.",
    },
    {
      question: "What support do I get as a seller?",
      answer:
        "You get 24/7 customer support via chat, email, or phone. Plus access to our seller academy with tutorials, best practices, and tips to grow your business.",
    },
  ];

  return (
    <View style={styles.container}>
      <StatusBar style="light" />

      {/* Header */}
      <LinearGradient
        colors={[colors.primary, colors.primaryDark]}
        style={styles.header}
      >
        <SafeAreaView edges={["top"]}>
          <View style={styles.headerContent}>
            <TouchableOpacity
              style={styles.backButton}
              onPress={() => router.back()}
            >
              <Ionicons name="arrow-back" size={24} color={colors.white} />
            </TouchableOpacity>
            <Text style={styles.headerTitle}>Why Sell on Kakebe?</Text>
            <View style={{ width: 40 }} />
          </View>
        </SafeAreaView>
      </LinearGradient>

      <ScrollView
        style={styles.scrollView}
        showsVerticalScrollIndicator={false}
        contentContainerStyle={styles.scrollContent}
      >
        {/* Hero Stats */}
        <View style={styles.heroStats}>
          <View style={styles.heroStatItem}>
            <Text style={styles.heroStatNumber}>10,000+</Text>
            <Text style={styles.heroStatLabel}>Active Buyers</Text>
          </View>
          <View style={styles.heroStatDivider} />
          <View style={styles.heroStatItem}>
            <Text style={styles.heroStatNumber}>500+</Text>
            <Text style={styles.heroStatLabel}>Sellers Earning</Text>
          </View>
          <View style={styles.heroStatDivider} />
          <View style={styles.heroStatItem}>
            <Text style={styles.heroStatNumber}>95%</Text>
            <Text style={styles.heroStatLabel}>Satisfaction</Text>
          </View>
        </View>

        {/* Main Benefits */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Everything You Need to Succeed</Text>

          <View style={styles.featuresGrid}>
            <FeatureCard
              icon={
                <Ionicons
                  name="pricetag-outline"
                  size={32}
                  color={colors.primary}
                />
              }
              title="Zero Listing Fees"
              description="List unlimited products for free. Only pay 5% commission on sales."
              badge="Popular"
            />

            <FeatureCard
              icon={
                <Ionicons name="people-outline" size={32} color={colors.primary} />
              }
              title="Huge Audience"
              description="Get instant access to 10,000+ active buyers searching daily."
            />

            <FeatureCard
              icon={
                <Ionicons
                  name="shield-checkmark-outline"
                  size={32}
                  color={colors.primary}
                />
              }
              title="Secure Payments"
              description="Get paid safely via mobile money or bank transfer within 48 hours."
            />

            <FeatureCard
              icon={
                <Ionicons
                  name="trending-up-outline"
                  size={32}
                  color={colors.primary}
                />
              }
              title="Sales Analytics"
              description="Track performance with detailed insights and reports."
            />

            <FeatureCard
              icon={
                <MaterialCommunityIcons
                  name="truck-fast-outline"
                  size={32}
                  color={colors.primary}
                />
              }
              title="Easy Shipping"
              description="Integrated delivery partners with discounted rates."
              badge="New"
            />

            <FeatureCard
              icon={
                <Ionicons
                  name="chatbubbles-outline"
                  size={32}
                  color={colors.primary}
                />
              }
              title="24/7 Support"
              description="Get help anytime via chat, email, or phone support."
            />
          </View>
        </View>

        {/* Comparison Section */}
        <View style={[styles.section, styles.comparisonSection]}>
          <Text style={styles.sectionTitle}>How We Compare</Text>
          <Text style={styles.sectionSubtitle}>
            See why sellers choose Kakebe over other platforms
          </Text>

          <View style={styles.comparisonTable}>
            <View style={styles.comparisonHeader}>
              <Text style={styles.comparisonHeaderText}>Feature</Text>
              <View style={styles.comparisonHeaderValues}>
                <View style={styles.comparisonHeaderCell}>
                  <MaterialCommunityIcons
                    name="store-check"
                    size={20}
                    color={colors.primary}
                  />
                  <Text style={styles.comparisonHeaderLabel}>Kakebe</Text>
                </View>
                <View style={styles.comparisonHeaderCell}>
                  <Text style={styles.comparisonHeaderLabelMuted}>Others</Text>
                </View>
              </View>
            </View>

            <ComparisonRow feature="Listing Fee" kakebe="Free" others="$0.20+" />
            <ComparisonRow feature="Commission" kakebe="5%" others="10-15%" />
            <ComparisonRow feature="Payment Time" kakebe="24-48h" others="7-14 days" />
            <ComparisonRow feature="Local Support" kakebe={true} others={false} />
            <ComparisonRow feature="Seller Academy" kakebe={true} others={false} />
            <ComparisonRow
              feature="Marketing Tools"
              kakebe={true}
              others="Paid"
            />
          </View>
        </View>

        {/* Success Stories */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Success Stories</Text>
          <Text style={styles.sectionSubtitle}>
            Real sellers sharing their experience
          </Text>

          <ScrollView
            horizontal
            showsHorizontalScrollIndicator={false}
            contentContainerStyle={styles.testimonialsContainer}
          >
            <TestimonialCard
              name="Sarah Nalubega"
              role="Fashion Boutique"
              avatar="SN"
              quote="I've tripled my sales in just 3 months. The platform is so easy to use and customer support is amazing!"
              stats="200% sales increase"
            />

            <TestimonialCard
              name="James Okello"
              role="Electronics Seller"
              avatar="JO"
              quote="Best decision for my business. I reach more customers than my physical store ever did."
              stats="500+ orders completed"
            />

            <TestimonialCard
              name="Grace Namukasa"
              role="Home Decor"
              avatar="GN"
              quote="The analytics help me understand what sells best. I've optimized my inventory and profits are up!"
              stats="₦2M+ in sales"
            />
          </ScrollView>
        </View>

        {/* Getting Started Steps */}
        <View style={[styles.section, styles.stepsSection]}>
          <Text style={styles.sectionTitle}>Get Started in 3 Easy Steps</Text>

          <View style={styles.stepsList}>
            <View style={styles.stepItem}>
              <View style={styles.stepCircle}>
                <Text style={styles.stepNumber}>1</Text>
              </View>
              <View style={styles.stepContent}>
                <Text style={styles.stepTitle}>Create Your Account</Text>
                <Text style={styles.stepDescription}>
                  Sign up with basic information. Takes less than 2 minutes.
                </Text>
              </View>
            </View>

            <View style={styles.stepConnector} />

            <View style={styles.stepItem}>
              <View style={styles.stepCircle}>
                <Text style={styles.stepNumber}>2</Text>
              </View>
              <View style={styles.stepContent}>
                <Text style={styles.stepTitle}>List Your First Product</Text>
                <Text style={styles.stepDescription}>
                  Add photos, description, and price. We'll guide you through it.
                </Text>
              </View>
            </View>

            <View style={styles.stepConnector} />

            <View style={styles.stepItem}>
              <View style={styles.stepCircle}>
                <Text style={styles.stepNumber}>3</Text>
              </View>
              <View style={styles.stepContent}>
                <Text style={styles.stepTitle}>Start Making Sales</Text>
                <Text style={styles.stepDescription}>
                  Your products go live instantly. Watch the orders roll in!
                </Text>
              </View>
            </View>
          </View>
        </View>

        {/* FAQ Section */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Frequently Asked Questions</Text>

          <View style={styles.faqList}>
            {faqs.map((faq, index) => (
              <FAQItem
                key={index}
                question={faq.question}
                answer={faq.answer}
                isExpanded={expandedFAQ === index}
                onToggle={() => toggleFAQ(index)}
              />
            ))}
          </View>

          <TouchableOpacity
            style={styles.supportLink}
            onPress={handleContactSupport}
          >
            <Ionicons name="help-circle-outline" size={20} color={colors.primary} />
            <Text style={styles.supportLinkText}>
              Still have questions? Contact Support
            </Text>
          </TouchableOpacity>
        </View>

        {/* Trust Badges */}
        <View style={styles.trustSection}>
          <View style={styles.trustBadges}>
            <View style={styles.trustBadge}>
              <Ionicons name="shield-checkmark" size={24} color={colors.success} />
              <Text style={styles.trustBadgeText}>Secure Platform</Text>
            </View>
            <View style={styles.trustBadge}>
              <Ionicons name="lock-closed" size={24} color={colors.success} />
              <Text style={styles.trustBadgeText}>Data Protected</Text>
            </View>
            <View style={styles.trustBadge}>
              <Ionicons name="checkmark-circle" size={24} color={colors.success} />
              <Text style={styles.trustBadgeText}>Verified Buyers</Text>
            </View>
          </View>
        </View>

        {/* Bottom Spacing */}
        <View style={{ height: 120 }} />
      </ScrollView>

      {/* Fixed CTA */}
      <View style={styles.ctaContainer}>
        <SafeAreaView edges={["bottom"]}>
          <TouchableOpacity
            style={styles.ctaButton}
            onPress={handleGetStarted}
            activeOpacity={0.8}
          >
            <LinearGradient
              colors={[colors.primary, colors.primaryDark]}
              style={styles.ctaGradient}
            >
              <Text style={styles.ctaButtonText}>Start Selling Now</Text>
              <Ionicons name="arrow-forward" size={22} color={colors.white} />
            </LinearGradient>
          </TouchableOpacity>
          <Text style={styles.ctaFootnote}>
            Free forever • No credit card required
          </Text>
        </SafeAreaView>
      </View>
    </View>
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: colors.white,
  },

  // Header
  header: {
    paddingBottom: spacingY._20,
  },
  headerContent: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    paddingHorizontal: spacingX._20,
    paddingTop: spacingY._12,
  },
  backButton: {
    width: 40,
    height: 40,
    alignItems: "center",
    justifyContent: "center",
  },
  headerTitle: {
    fontSize: fontSize.xl,
    fontWeight: fontWeight.bold,
    color: colors.white,
  },

  // Hero Stats
  heroStats: {
    flexDirection: "row",
    backgroundColor: colors.white,
    marginHorizontal: spacingX._20,
    marginTop: -30,
    borderRadius: borderRadius.lg,
    padding: spacingX._20,
    ...shadow.lg,
  },
  heroStatItem: {
    flex: 1,
    alignItems: "center",
  },
  heroStatNumber: {
    fontSize: fontSize.xxl,
    fontWeight: fontWeight.bold,
    color: colors.primary,
  },
  heroStatLabel: {
    fontSize: fontSize.xs,
    color: colors.textMuted,
    marginTop: spacingY._4,
    textAlign: "center",
  },
  heroStatDivider: {
    width: 1,
    backgroundColor: colors.border,
    marginHorizontal: spacingX._8,
  },

  // Scroll View
  scrollView: {
    flex: 1,
  },
  scrollContent: {
    paddingTop: spacingY._50
  },

  // Section
  section: {
    paddingHorizontal: spacingX._20,
    paddingVertical: spacingY._24,
  },
  sectionTitle: {
    fontSize: fontSize.xxl,
    fontWeight: fontWeight.bold,
    color: colors.textPrimary,
    marginBottom: spacingY._8,
  },
  sectionSubtitle: {
    fontSize: fontSize.md,
    color: colors.textSecondary,
    marginBottom: spacingY._20,
  },

  // Features Grid
  featuresGrid: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 12,
  },
  featureCard: {
    width: (width - spacingX._20 * 2 - 12) / 2,
    backgroundColor: colors.white,
    borderRadius: borderRadius.lg,
    padding: spacingX._16,
    borderWidth: 1,
    borderColor: colors.border,
    position: "relative",
  },
  featureBadge: {
    position: "absolute",
    top: 8,
    right: 8,
    backgroundColor: colors.warning,
    paddingHorizontal: spacingX._8,
    paddingVertical: spacingY._2,
    borderRadius: borderRadius.sm,
  },
  featureBadgeText: {
    fontSize: fontSize.xs,
    fontWeight: fontWeight.semibold,
    color: colors.white,
  },
  featureIconContainer: {
    width: 56,
    height: 56,
    borderRadius: borderRadius.md,
    backgroundColor: colors.primarySoft,
    alignItems: "center",
    justifyContent: "center",
    marginBottom: spacingY._12,
  },
  featureTitle: {
    fontSize: fontSize.md,
    fontWeight: fontWeight.semibold,
    color: colors.textPrimary,
    marginBottom: spacingY._6,
  },
  featureDescription: {
    fontSize: fontSize.sm,
    color: colors.textSecondary,
    lineHeight: 18,
  },

  // Comparison Section
  comparisonSection: {
    backgroundColor: colors.backgroundSecondary,
  },
  comparisonTable: {
    backgroundColor: colors.white,
    borderRadius: borderRadius.lg,
    overflow: "hidden",
    borderWidth: 1,
    borderColor: colors.border,
  },
  comparisonHeader: {
    flexDirection: "row",
    backgroundColor: colors.primarySoft,
    padding: spacingX._16,
    borderBottomWidth: 1,
    borderBottomColor: colors.border,
  },
  comparisonHeaderText: {
    flex: 1,
    fontSize: fontSize.sm,
    fontWeight: fontWeight.semibold,
    color: colors.textPrimary,
  },
  comparisonHeaderValues: {
    flexDirection: "row",
    width: 200,
  },
  comparisonHeaderCell: {
    flex: 1,
    alignItems: "center",
    flexDirection: "row",
    justifyContent: "center",
    gap: 4,
  },
  comparisonHeaderLabel: {
    fontSize: fontSize.sm,
    fontWeight: fontWeight.bold,
    color: colors.primary,
  },
  comparisonHeaderLabelMuted: {
    fontSize: fontSize.sm,
    fontWeight: fontWeight.semibold,
    color: colors.textMuted,
  },
  comparisonRow: {
    flexDirection: "row",
    padding: spacingX._16,
    borderBottomWidth: 1,
    borderBottomColor: colors.border,
  },
  comparisonFeature: {
    flex: 1,
    fontSize: fontSize.md,
    color: colors.textPrimary,
  },
  comparisonValues: {
    flexDirection: "row",
    width: 200,
  },
  comparisonCell: {
    flex: 1,
    alignItems: "center",
    justifyContent: "center",
  },
  comparisonText: {
    fontSize: fontSize.md,
    fontWeight: fontWeight.semibold,
    color: colors.primary,
  },
  comparisonTextMuted: {
    color: colors.textMuted,
    fontWeight: fontWeight.medium,
  },

  // Testimonials
  testimonialsContainer: {
    paddingRight: spacingX._20,
    gap: 12,
  },
  testimonialCard: {
    width: width * 0.8,
    backgroundColor: colors.white,
    borderRadius: borderRadius.lg,
    padding: spacingX._20,
    borderWidth: 1,
    borderColor: colors.border,
    position: "relative",
  },
  quoteIconContainer: {
    position: "absolute",
    top: -10,
    left: 20,
    width: 32,
    height: 32,
    borderRadius: 16,
    backgroundColor: colors.primarySoft,
    alignItems: "center",
    justifyContent: "center",
  },
  testimonialQuote: {
    fontSize: fontSize.md,
    color: colors.textSecondary,
    lineHeight: 22,
    fontStyle: "italic",
    marginTop: spacingY._16,
    marginBottom: spacingY._16,
  },
  testimonialFooter: {
    flexDirection: "row",
    alignItems: "center",
    marginBottom: spacingY._12,
  },
  testimonialAvatar: {
    width: 40,
    height: 40,
    borderRadius: 20,
    backgroundColor: colors.primary,
    alignItems: "center",
    justifyContent: "center",
    marginRight: spacingX._12,
  },
  testimonialAvatarText: {
    fontSize: fontSize.md,
    fontWeight: fontWeight.bold,
    color: colors.white,
  },
  testimonialInfo: {
    flex: 1,
  },
  testimonialName: {
    fontSize: fontSize.md,
    fontWeight: fontWeight.semibold,
    color: colors.textPrimary,
  },
  testimonialRole: {
    fontSize: fontSize.sm,
    color: colors.textMuted,
  },
  testimonialStats: {
    flexDirection: "row",
    alignItems: "center",
    backgroundColor: colors.white,
    paddingHorizontal: spacingX._12,
    paddingVertical: spacingY._6,
    borderRadius: borderRadius.md,
    alignSelf: "flex-start",
  },
  testimonialStatsText: {
    fontSize: fontSize.sm,
    fontWeight: fontWeight.semibold,
    color: colors.success,
    marginLeft: spacingX._4,
  },

  // Steps Section
  stepsSection: {
    backgroundColor: colors.backgroundSecondary,
  },
  stepsList: {
    paddingHorizontal: spacingX._10,
  },
  stepItem: {
    flexDirection: "row",
    alignItems: "flex-start",
  },
  stepCircle: {
    width: 48,
    height: 48,
    borderRadius: 24,
    backgroundColor: colors.primary,
    alignItems: "center",
    justifyContent: "center",
    marginRight: spacingX._16,
    ...shadow.md,
  },
  stepNumber: {
    fontSize: fontSize.xl,
    fontWeight: fontWeight.bold,
    color: colors.white,
  },
  stepContent: {
    flex: 1,
    paddingTop: spacingY._4,
  },
  stepTitle: {
    fontSize: fontSize.lg,
    fontWeight: fontWeight.semibold,
    color: colors.textPrimary,
    marginBottom: spacingY._6,
  },
  stepDescription: {
    fontSize: fontSize.md,
    color: colors.textSecondary,
    lineHeight: 20,
  },
  stepConnector: {
    width: 2,
    height: 40,
    backgroundColor: colors.primary,
    opacity: 0.3,
    marginLeft: 23,
    marginVertical: spacingY._8,
  },

  // FAQ Section
  faqList: {
    gap: 12,
  },
  faqItem: {
    backgroundColor: colors.white,
    borderRadius: borderRadius.lg,
    padding: spacingX._16,
    borderWidth: 1,
    borderColor: colors.border,
  },
  faqHeader: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  },
  faqQuestion: {
    flex: 1,
    fontSize: fontSize.md,
    fontWeight: fontWeight.semibold,
    color: colors.textPrimary,
    marginRight: spacingX._12,
  },
  faqAnswer: {
    fontSize: fontSize.md,
    color: colors.textSecondary,
    lineHeight: 22,
    marginTop: spacingY._12,
    paddingTop: spacingY._12,
    borderTopWidth: 1,
    borderTopColor: colors.border,
  },
  supportLink: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    marginTop: spacingY._20,
    padding: spacingX._16,
  },
  supportLinkText: {
    fontSize: fontSize.md,
    fontWeight: fontWeight.semibold,
    color: colors.primary,
    marginLeft: spacingX._8,
  },

  // Trust Section
  trustSection: {
    paddingHorizontal: spacingX._20,
    paddingVertical: spacingY._24,
    backgroundColor: colors.backgroundSecondary,
  },
  trustBadges: {
    flexDirection: "row",
    justifyContent: "space-around",
    gap: 12,
  },
  trustBadge: {
    flex: 1,
    alignItems: "center",
    backgroundColor: colors.white,
    paddingVertical: spacingY._16,
    borderRadius: borderRadius.lg,
    borderWidth: 1,
    borderColor: colors.border,
  },
  trustBadgeText: {
    fontSize: fontSize.xs,
    color: colors.textSecondary,
    marginTop: spacingY._6,
    textAlign: "center",
  },

  // CTA Container
  ctaContainer: {
    position: "absolute",
    bottom: 0,
    left: 0,
    right: 0,
    backgroundColor: colors.white,
    paddingHorizontal: spacingX._20,
    paddingTop: spacingY._16,
    borderTopWidth: 1,
    borderTopColor: colors.border,
    ...shadow.lg,
  },
  ctaButton: {
    borderRadius: borderRadius.lg,
    overflow: "hidden",
    ...shadow.primary,
  },
  ctaGradient: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    paddingVertical: spacingY._16,
    gap: 8,
  },
  ctaButtonText: {
    fontSize: fontSize.lg,
    fontWeight: fontWeight.bold,
    color: colors.white,
  },
  ctaFootnote: {
    fontSize: fontSize.sm,
    color: colors.textMuted,
    textAlign: "center",
    marginTop: spacingY._12,
    marginBottom: spacingY._4,
  },
});